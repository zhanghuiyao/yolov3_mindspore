...
def create_train_static_shape_fn(network, optimizer, config):
    ...
    if is_gpu:
        ...
        loss_scale_value = config.loss_scale

    loss_scaler = StaticLossScaler(loss_scale_value)
...
def create_train_static_shape_fn_with_sens(network, optimizer, config):
    ...
    def train_step(*inputs):
        loss = network(*inputs)
        sens = ops.fill(loss.dtype, loss.shape, loss_scale_value)
...
def run_train():
    ...
    lr = get_lr(config)
    opt = nn.Momentum(params=get_param_groups(network), momentum=config.momentum, learning_rate=Tensor(lr),
    ...
    if config.ms_strategy == "StaticCell":
        ...
        if config.ms_use_sens:
            train_step = create_train_static_shape_fn_with_sens(network, opt, config)
        else:
    ...
    for epoch_idx in range(config.max_epoch):
        for step_idx, data in enumerate(data_loader):
            ...
            loss = train_step(images, batch_y_true_0, batch_y_true_1, batch_y_true_2,
